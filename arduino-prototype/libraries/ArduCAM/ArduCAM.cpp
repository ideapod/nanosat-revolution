...
void ArduCAM::diagnosticsOn()
{
	diagFlag = true;
}

void ArduCAM::diagnosticsOff()
{
	diagFlag = false;
}

void ArduCAM::diagmsg(char *msg)
{
	if (diagFlag == true) {
		Serial.println(msg);
	}
...
void ArduCAM::diagReadBusMsg(int address, int value)
{
	diagBusMsg((char *)"read bus(hex):",address, value);	
} 

void ArduCAM::diagWriteBusMsg(int address, int value)
{
	diagBusMsg((char *)"write bus(hex):",address, value);	
} 

void ArduCAM::diagBusMsg(char *msg, int address, int value)
{
	char stringa[250];

	sprintf(stringa, "%s: address: %x, value: %x", msg,address, value);

	diagmsg((char *)stringa);	
} 
...
uint8_t ArduCAM::bus_read(int address) {
  ...
  #if defined(ESP8266) && defined(OV5642_CAM)
  ...
  diagReadBusMsg(address, (int) value);
  return value;
}
...
boolean ArduCAM::isCapturing()
{
	return capturing;
}

boolean ArduCAM::isCaptureComplete()
{
	int fifoFlag = get_bit(ARDUCHIP_TRIG, CAP_DONE_MASK);
	if (fifoFlag == CAP_DONE_MASK) {
		diagmsg((char *)"Capture Complete");
		capturing = false;
		return true;
	}
	else
...
void ArduCAM::complete_capture(void)
{
      ...
      set_bit(0x41,0x01); // clear capture flag
...
*/
	...
	diagmsg((char *)"set bit to start capture");
	capturing = true;
	write_reg(ARDUCHIP_FIFO, FIFO_START_MASK);
	diagmsg((char *)"hack: telling FIFO control to start capture");
	write_reg(0x04,0b00000010);
}
